import 'package:flutter/material.dart';

class AppColor {
  static const Color primaryColor = Color(0xff3085FE);
  static const Color textColor = Color(0xffBCBEC3);
  static const Color white = Color(0xffFFFFFF);
  static const Color buttonWhite = Color(0xffEEEFF0);
  static const Color black = Colors.black;
  static const Color greyText = Color(0xff797979);
  static const Color bgColor = Color(0xffFFFDF6);
  static const Color borderColor = Color(0xffE2E2E2);
  static const Color darkGrey = Color(0xff767676);
  static const Color searchColor = Color(0XffFFEBEBEB);
  static const Color transparent = Colors.transparent;
}
